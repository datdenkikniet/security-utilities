...
pub const VERSION_TWO_CHECKSUM_SEED: u64 = compute_his_v1_checksum_seed_from_array(b"Default0");
...
pub fn try_validate_common_annotated_key_valid_signature(key: &str) -> bool {
    ...
    let encoded = &checksum_bytes[..checksum_len];
...
///
/// This function will return an error if the `versioned_key_kind` does not meet the required criteria.
pub const fn compute_his_v1_checksum_seed_from_array(versioned_key_kind: &[u8; 8]) -> u64 {

    if versioned_key_kind[7] < b'0' || versioned_key_kind[7] > b'9' {
        panic!("The versioned literal must end with a digit.");
    }
